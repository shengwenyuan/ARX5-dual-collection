...
def load_config(path: Path) -> CompositionConfig:
    ...
    for index, raw in enumerate(source_rows):
        ...
        _keys(row, {"name", "path", "select_all", "selection_manifest"}, f"sources[{index}]")
        ...
        if manifest_value is not None:
            manifest = Path(_string(manifest_value, f"sources[{index}].selection_manifest"))
    ...
    if runtime_row is not None:
        ...
        runtime = V3RuntimeConfig(_absolute_path(runtime_object.get("python"), "v3_runtime.python"))
