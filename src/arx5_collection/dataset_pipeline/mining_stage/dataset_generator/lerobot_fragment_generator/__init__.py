...
from .discovery import episode_lookup
from .images import extract_selected_rgb
from .video import configured_lerobot_encoder
...
def export_lerobot(
    ...
    *,
    ...
    final_root = (
        dataset_root if dataset_root is not None else output_root / "lerobot" / repo_id
    )
    ...
    try:
        with staged_directory(final_root, precreate=False) as temporary_root:
            ...
            with configured_lerobot_encoder(video, phase_reporter):
                for episode_id, episode_segments in segments_by_episode.items():
                    ...
                    image_paths = extract_selected_rgb(
                        source_episode, refs, episode_cache
                    )
        ...
        if source_manifest:
            source_by_segment = {str(row["segment_id"]): row for row in source_manifest}
...
def run(
    ...
    selection = require_output(context.selection, "trajectory_labeler")
    if selection.output_dir is None:
        raise RuntimeError("trajectory_labeler did not persist action mining artifacts")
    context.dataset_root = timed(
        unit.type,
        lambda: export_lerobot(
            ...
            context.output_root,
            context.repo_id,
            dataset_root=context.output_root / "lerobot",
            video=context.recipe.video,
            phase_reporter=lambda name, seconds: context.reported_phase_seconds.append(
                (f"{unit.type}.{name}", seconds)
            ),
        ),
    )
