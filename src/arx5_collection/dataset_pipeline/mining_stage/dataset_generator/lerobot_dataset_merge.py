...
from arx5_collection.dataset_pipeline.execution.models import JobState
from arx5_collection.dataset_pipeline.execution.models import SelectionEntry
from arx5_collection.dataset_pipeline.persistence.fragment import (
    FRAGMENT_SCHEMA_VERSION,
)
from arx5_collection.dataset_pipeline.persistence.fragment import (
    load_committed_fragment,
)
from arx5_collection.dataset_pipeline.persistence.manifest import RunManifest
...
class DatasetGeneratorContext:
    manifest: RunManifest
    temporary: Path
    output_path: Path
    recipe: DatasetPipelineRecipe
    repo_id: str
    committed: tuple[SelectionEntry, ...] | None = None
    omitted: tuple[SelectionEntry, ...] | None = None
    descriptors: tuple[FragmentDescriptor, ...] | None = None
...
def build_lerobot_v21_snapshot(
    ...
    from .registry import DATASET_UNIT_RUNNERS

    units = tuple(
        unit
        for unit in recipe.pipeline.stage("dataset_generator").units
        if unit.type in DATASET_UNIT_RUNNERS
    )
    with staged_directory(output_path) as temporary:
        context = DatasetGeneratorContext(
            manifest,
            temporary,
            output_path,
            recipe,
        ...
        for unit in units:
            DATASET_UNIT_RUNNERS[unit.type](context, unit)

    if (
        context.committed is None
        or context.descriptors is None
        or context.tasks is None
        or context.build is None
        or context.validation is None
    ):
        raise RuntimeError("Dataset generator did not produce a validated snapshot")

    _remove_run_cache(manifest.run_dir)
    return SnapshotBuildResult(
        output_path=output_path,
        repo_id=repo_id,
        source_episode_count=len(context.committed),
        fragment_count=len(context.descriptors),
        episode_count=context.build["episode_count"],
        frame_count=context.build["frame_count"],
        tasks=context.tasks,
    )
...
def _validate_fragment_dataset(
    ...
    observed_indices = {
        _non_negative_int(row.get("episode_index"), "episode_index") for row in episodes
    }
    ...
    if (
        info.get("total_episodes") != episode_count
    ...
    if (
        sum(_positive_int(row.get("length"), "episode length") for row in episodes)
        != frame_count
    ):
...
def _validate_info_contract(
    ...
    for key in ("observation.state", "action"):
        ...
        if not isinstance(feature, dict) or feature.get("names") != [order]:
            raise ValueError(
                f"LeRobot feature disagrees with state/action order: {key}"
            )
...
def _validate_fragment_compatibility(
    ...
    for descriptor in descriptors:
        if _contract_signature(descriptor.fragment) != baseline_contract:
            raise ValueError(
                f"Fragment training contract drift: {descriptor.episode_id}"
            )
...
def _assemble_v21(
    ...
    for descriptor in descriptors:
        ...
        for episode in descriptor.episodes:
            ...
            target_data = target / _format_path(baseline, "data_path", global_episode)
            ...
            episode_tasks = [
                _string(value, "episode task") for value in episode["tasks"]
            ]
            if set(mapped_task_indices) != {
                task_indices[task] for task in episode_tasks
            }:
...
def _rewrite_episode_parquet(
    ...
    for value in table["task_index"].to_pylist():
        try:
            ...
            raise ValueError(
                f"Parquet references an unknown task_index: {value!r}"
            ) from error
    replacements = {
...
def run(context: DatasetGeneratorContext, unit: UnitSpec) -> None:
    committed, omitted = _partition_selection(context.manifest)
    descriptors = tuple(
        _load_fragment(context.manifest.run_dir, selection, context.recipe)
