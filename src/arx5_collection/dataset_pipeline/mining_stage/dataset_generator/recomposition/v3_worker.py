...
def _inspect(name: str, root: Path) -> dict[str, Any]:
    ...
    if (
        snapshot.get("status") != "committed"
        or snapshot.get("builder_backend") != "lerobot-v3.0"
    ):
...
def _compose(request: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, source in enumerate(request["sources"]):
        ...
        if source["backend"] == "lerobot-v2.1":
            convert_dataset(
                repo_id, root=root, push_to_hub=False, force_conversion=True
            )
        ...
        for group_index, group in enumerate(groups):
            ...
            selected = split_dataset(
                dataset, {"selected": group}, output_dir=split_root
            )["selected"]
    ...
    wanted = (
        int(expected["episodes"]),
        int(expected["frames"]),
        list(expected["tasks"]),
    )
    if observed != wanted:
        raise ValueError(
            f"v3 aggregate totals disagree with plan: {observed} != {wanted}"
        )
    video_files = (
        len(list((output / "videos").glob("*/*/*.mp4")))
...
def _ordered_whole_shard_groups(meta, indices: list[int]) -> list[list[int]]:
    ...
    while cursor < len(indices):
        ...
        if indices[cursor : cursor + len(component)] != component:
            raise ValueError(
                "cannot reorder Episodes within a shared video-shard component"
            )
