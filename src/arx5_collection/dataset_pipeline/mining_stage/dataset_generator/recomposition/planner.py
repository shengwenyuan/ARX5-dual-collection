...
def build_plan(config: CompositionConfig) -> CompositionPlan:
    ...
    for source_config, descriptor in zip(config.sources, descriptors, strict=True):
        episodes = (
            descriptor.episodes
            if source_config.select_all
            else _select_manifest(descriptor, source_config.selection_manifest)
        )
    ...
    return CompositionPlan(
        config, selected_tuple, tasks, frames, videos, contract, fingerprint
    )


def _read_source(
    config: CompositionConfig, name: str, root: Path
) -> SnapshotDescriptor:
...
def _select_manifest(
    ...
    for row in read_jsonl(manifest_path):
        ...
        try:
            ...
            raise ValueError(
                f"selection segment is absent from source: {segment_id}"
            ) from error
        expected = (
...
def _logical_features(value: object) -> object:
    ...
    for name, feature in value.items():
        if not isinstance(feature, dict):
            ...
            continue
        result[name] = {
            key: feature[key] for key in ("dtype", "shape", "names") if key in feature
        }
    return result
...
def _validate_task_scope(selected: tuple[SelectedEpisode, ...]) -> None:
    ...
    for item in selected:
        episode_id = _string(
            item.episode.provenance.get("source_episode_id"), "source_episode_id"
        )
...
def _validate_v3_shard_selection(selected: tuple[SelectedEpisode, ...]) -> None:
    ...
    for name, source in sources.items():
        if source.backend != "lerobot-v3.0" or len(chosen[name]) == len(
            source.episodes
        ):
        ...
        for episode in source.episodes:
            ...
            for key, pair in videos.items():
                if not isinstance(pair, list) or len(pair) != 2:
                    raise ValueError(
                        f"v3 video shard identity is invalid: {source.root}"
                    )
...
def _validate_v3_component_order(
    ...
    parent = {
        episode.episode_index: episode.episode_index for episode in source.episodes
    }
