...
def read_v21_snapshot(name: str, root: Path) -> SnapshotDescriptor:
    ...
    stats = _indexed(
        read_jsonl(root / "meta" / "episodes_stats.jsonl"), "episode_index", "stats"
    )
    tasks = _task_map(read_jsonl(root / "meta" / "tasks.jsonl"))
    provenance = _indexed(
        read_jsonl(manifest_path), "lerobot_episode_index", "source manifest"
    )
    expected = set(range(len(episode_rows)))
    episodes_by_index = _indexed(episode_rows, "episode_index", "episodes")
    if (
        set(episodes_by_index) != expected
        or set(stats) != expected
        or set(provenance) != expected
    ):
        raise ValueError(
            f"source Episode metadata is not contiguous and complete: {root}"
        )
...
def _assemble(
    target: Path, plan: CompositionPlan, baseline: dict[str, Any]
) -> dict[str, Any]:
    ...
    for global_episode, item in enumerate(plan.selected):
        ...
        for key in video_keys:
            ...
            target_video = target / format_v21_path(
                baseline, "video_path", global_episode, key
            )
            ...
            "sources": [
                {
                    ...
                    "selected_episodes": sum(
                        item.source.name == source.name for item in plan.selected
                    ),
                }
    ...
    if isinstance(recipe, dict):
        ...
        if isinstance(sampling, dict):
            ...
            if isinstance(value, int) and not isinstance(value, bool) and value > 0:
                return value
    return PI05_ARX5_SPEC.dataset.action_horizon
