...
V3_LEROBOT_VERSION = PI05_ARX5_SPEC.dataset.lerobot_v3_version
V3_LEROBOT_COMMIT = PI05_ARX5_SPEC.dataset.lerobot_v3_commit
...
class V3WorkerClient:
    ...
    def _call(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory(
            prefix="arx5-lerobot-v3-protocol-"
        ) as temporary:
            root = Path(temporary)
            ...
            completed = subprocess.run(
                [
                    str(self.python),
                    "-m",
                    "arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v3_worker",
                    str(request_path),
                    str(result_path),
                ],
                text=True,
...
def build_v3(plan: CompositionPlan) -> CompositionResult:
    if plan.config.output.backend != "lerobot-v3.0" or plan.config.v3_runtime is None:
        raise ValueError(
            "v3 backend requires output.backend=lerobot-v3.0 and v3_runtime"
        )
    ...
    with tempfile.TemporaryDirectory(
        prefix=f".{output.name}.v3-inputs.", dir=output.parent
    ) as workspace_value:
...
def _materialize_worker_sources(
    plan: CompositionPlan, workspace: Path
) -> list[dict[str, Any]]:
    ...
    for source, selected in grouped:
        if source.backend == "lerobot-v2.1":
            ...
            build_v21(
                source_plan,
                validator=lambda *_args, **_kwargs: {"status": "staged-for-v3"},
            )
            result.append(
                {
                    "name": source.name,
            ...
            result.append(
                {
                    ...
                    "episode_indices": [
                        item.episode.episode_index for item in selected
                    ],
                }
            )
    return result
...
def _write_v3_sidecars(
    target: Path, plan: CompositionPlan, worker: dict[str, Any]
) -> None:
...
def _validate_worker_result(plan: CompositionPlan, result: dict[str, Any]) -> None:
    ...
    if observed != expected:
        raise ValueError(
            f"v3 worker result does not match frozen plan: {observed} != {expected}"
        )
