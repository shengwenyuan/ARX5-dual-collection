...
def _pair_same_camera(
    ...
    for stamp in sorted(set(colors) | set(depths)):
        ...
        pairs.extend(
            ImagePair(color_items[index], depth_items[index])
...
def _nearest_pair(
    ...
    selected = min(
        candidates, key=lambda pair: (abs(pair.stamp_ns - target_ns), pair.stamp_ns)
    )
...
def build_frame_groups(
    ...
    for role in ("left", "right", "overview"):
        ...
        if camera_topic(role, "aligned_depth") not in required_topics(
            scan.capture_profile
        ):
    ...
    left_pairs = tuple(
        sorted(paired["left"], key=lambda pair: (pair.stamp_ns, pair.color.sequence))
    )
    right_pairs = tuple(
        sorted(paired["right"], key=lambda pair: (pair.stamp_ns, pair.color.sequence))
    )
    overview_pairs = tuple(
        sorted(
            paired["overview"], key=lambda pair: (pair.stamp_ns, pair.color.sequence)
        )
    )
    ...
    for overview in overview_pairs:
        ...
        selected_left_arm = _latest_arm(
            left_arm, left_arm_stamps, cutoff, policy.arm_max_age_ns
        )
        selected_right_arm = _latest_arm(
            right_arm, right_arm_stamps, cutoff, policy.arm_max_age_ns
        )
...
def run(
    ...
    context.pairing = timed(
        unit.type,
        lambda: build_frame_groups(sanitycheck.scan, context.recipe.cleaning),
    )
