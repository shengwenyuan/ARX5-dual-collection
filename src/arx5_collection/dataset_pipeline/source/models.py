...
def camera_topic(role: str, leaf: str) -> str:
    return RGBD_STREAMS[f"camera_{role}_{leaf}"]


RGBD_STREAMS = stream_contract(CaptureProfile.RGBD)
LEFT_ARM_TOPIC = RGBD_STREAMS["left_arm_state"]
RIGHT_ARM_TOPIC = RGBD_STREAMS["right_arm_state"]
...
class FrameGroup:
    ...
    def __post_init__(self) -> None:
        ...
        latest_image = max(
            self.overview.stamp_ns, self.left.stamp_ns, self.right.stamp_ns
        )
        if self.observation_cutoff_ns != latest_image:
            raise ValueError(
                "observation_cutoff_ns must be the latest selected image timestamp"
            )
...
@dataclass(frozen=True, slots=True)
class EpisodeSanitycheckResult:
    metadata: dict[str, Any]
    scan: EpisodeScan
    timeline: dict[str, dict[str, int | None]]
    timeline_has_errors: bool
