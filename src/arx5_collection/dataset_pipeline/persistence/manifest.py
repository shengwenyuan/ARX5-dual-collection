...
from arx5_collection.dataset_pipeline.execution.models import JobEvent
from arx5_collection.dataset_pipeline.execution.models import JobSnapshot
from arx5_collection.dataset_pipeline.execution.models import JobState
from arx5_collection.dataset_pipeline.execution.models import RunDefinition
...
class RunManifest:
    ...
    def transition(
        ...
        *,
        ...
        if current.state in _TERMINAL:
            raise ValueError(
                f"job {episode_id} is terminal in state={current.state.value}"
            )
    ...
    def retry_failed(
        self, episode_id: str, *, detail: str | None = None
    ) -> JobSnapshot:
        current = self._job(episode_id)
...
def _replay_events(
    ...
    for index, event in enumerate(events):
        ...
        if event.episode_id not in selected_ids:
            raise ValueError(
                f"job event references unselected Episode: {event.episode_id}"
            )
        ...
        if current is None:
            ...
            if (
                event.state is not JobState.STAGING
                or event.attempt != current.attempt + 1
            ):
...
def _run_definition(value: dict[str, Any], schema_version: int) -> RunDefinition:
    ...
    if not all(
        path.is_absolute() for path in (source_root, streaming_root, output_path)
    ):
