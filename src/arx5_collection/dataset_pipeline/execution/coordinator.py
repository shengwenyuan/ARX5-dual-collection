...
from arx5_collection.dataset_pipeline.configuration.run import RuntimeSettings
...
class StreamingCoordinator:
    ...
    def _fill_legacy(
        ...
        while len(active) < workers and (ready_convert or ready_stage):
            ...
            active[future] = ActiveWork(episode_id, WorkPhase.STAGE, time.monotonic())
            ...
            with self._conversion_executor_factory(
                ...
                while ready_stage or ready_convert or active:
                    ...
                    if not active:
                        ...
                        if ready_convert:
                            raise RuntimeError(
                                "conversion queue could not be scheduled"
                            )
                        break
    ...
    def _fill_conversions(
        ...
        active_count = sum(work.phase is WorkPhase.CONVERT for work in active.values())
