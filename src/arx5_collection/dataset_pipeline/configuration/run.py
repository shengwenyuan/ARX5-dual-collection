...
class DatasetPipelineConfig:
    ...
    @classmethod
    def load(cls, path: str | Path) -> DatasetPipelineConfig:
        with Path(path).open("rb") as stream:
            payload = tomllib.load(stream)
        return cls.from_mapping(payload)

    @classmethod
    def from_mapping(cls, payload: dict[str, object]) -> DatasetPipelineConfig:
        _exact_keys(
            payload,
        ...
        if (
            ...
            raise ValueError(
                "dataset pipeline config schema_version must be 1, 2, or 3"
            )
        ...
        dataset_name = _single_component(output["dataset_name"], "output.dataset_name")
...
def _runtime_config(
    ...
    high_bytes = _positive_int(value["ready_high_bytes"], "runtime.ready_high_bytes")
...
def _table(payload: dict[str, object], name: str) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"dataset pipeline config must contain a [{name}] table")
    return value
...
def _require_within(path: Path, root: Path, label: str) -> None:
    ...
    if (
        ...
        or normalized_root not in normalized_path.parents
    ):
...
def _include_paths(value: object) -> tuple[Path, ...]:
    ...
    for item in value:
        ...
        if path.is_absolute() or any(part in {"", ".", ".."} for part in path.parts):
            raise ValueError(
                "source.include_paths entries must be normalized relative paths"
            )
        paths.append(path)
