...
def usb_inventory(root: Path = Path("/sys/bus/usb/devices")) -> list[dict[str, Any]]:
    return [
        device
...
def can_inventory() -> list[dict[str, Any]]:
    ...
    return [
        ...
        if entry.get("ifname")
        and entry.get("linkinfo", {}).get("info_kind") == "can"
    ]
    ...
    for device in rs.context().query_devices():
        ...
        for key, name in (
            ...
            try:
                ...
                info[name] = None
        info["sensors"] = [sensor_capabilities(rs, sensor) for sensor in device.query_sensors()]
        devices.append(info)
    return {
        "available": True,
