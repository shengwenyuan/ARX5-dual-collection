...
from .models import DiscoveryResult, EpisodeCandidate, FileIdentity
...
def _walk(
    ...
    if directory in visited:
        raise ValueError(f"source directory is reachable through multiple paths: {directory}")
...
def _candidate(
    ...
    try:
        ...
        raise ValueError(f"invalid Episode metadata: {metadata_path}: {error}") from error
...
def _resolve_inside(path: Path, source_root: Path) -> Path:
    ...
    try:
        ...
        raise ValueError(f"source path escapes configured root: {path} -> {resolved}") from error
    return resolved
