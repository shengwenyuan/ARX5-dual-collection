...
from arx5_collection.production.config import PedalConfig, TriggerConfig
...
class PedalIdentifier:
            ...
            for candidate in candidates:
                self._opened.append(
                    _OpenedPedal(
                        candidate,
                        self.open_function(
                            candidate.path, os.O_RDONLY | os.O_NONBLOCK
                        ),
                    )
                )
    ...
    def wait_for_press(
        ...
        try:
            ...
            raise PedalIdentificationError(f"cannot read pedal report: {error}") from error
        identities = {
    ...
    def identify(
        ...
        prompt("abort")
        abort = self.wait_for_press(
            timeout_s, frozenset({activate.serial_number})
        )
