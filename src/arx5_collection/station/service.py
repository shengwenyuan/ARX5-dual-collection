...
class StationInitializationService:
    ...
    def _validate_inventory(inventory: StationInventory) -> None:
        if len(inventory.usb2can) != 2:
            raise StationInitializationError(
                f"expected exactly two USB2CAN devices, found {len(inventory.usb2can)}"
            )
        if len(inventory.cameras) != 3:
            raise StationInitializationError(
                f"expected exactly three D405 cameras, found {len(inventory.cameras)}"
            )
        pedal_identities = {
            (pedal.vendor_id, pedal.product_id, pedal.serial_number)
            for pedal in inventory.pedals
        }
        if len(pedal_identities) != 2:
            raise StationInitializationError(
                f"expected exactly two stable pedals, found {len(pedal_identities)}"
            )
