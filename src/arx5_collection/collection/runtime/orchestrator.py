...
from arx5_collection.collection.metadata import MetadataContext
from arx5_collection.collection.episode.finalization import McapFinalizer
...
class ProductionSession:
    ...
    def __init__(
        ...
        min_free_bytes: int = ENVIRONMENT.session.min_free_gib * GIB,
        readiness_timeout_s: float = ENVIRONMENT.session.readiness_timeout_s,
        ...
        fail_directory: str = ENVIRONMENT.session.fail_directory,
        compression_enabled: bool = ENVIRONMENT.session.compression_enabled,
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.supervisor.start(self.commands.arm_state_adapter(self.arm_profile))
            self._report(
    ...
    def create_runtime(
        ...
        def stop_episode(stopping: RecordingStopping) -> None:
            if recording_stopping_hook is not None:
                ...
                self._confirm_gravity_compensation(f"Episode {stopping.outcome.value}")
