...
from arx5_collection.collection.episode.cli import (
    non_negative_int,
    run_episode_loop,
)
...
from arx5_collection.collection.station.service import (
...
DEFAULT_STATION_CONFIG = ENVIRONMENT.paths.station_config
...
def build_parser() -> argparse.ArgumentParser:
    ...
    configure.add_argument(
        "--log-dir", type=Path, default=ENVIRONMENT.paths.station_log_dir
    )
    ...
    devices = subcommands.add_parser(
        "devices", help="verify configured device identities"
    )
    devices.add_argument("--station-config", type=Path, default=DEFAULT_STATION_CONFIG)
...
def add_session_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--min-free-gib",
        type=positive_int,
        default=ENVIRONMENT.session.min_free_gib,
    )
    parser.add_argument(
        "--readiness-timeout-s",
        type=positive_float,
        default=ENVIRONMENT.session.readiness_timeout_s,
    )
    parser.add_argument(
        "--no-compress",
        action="store_true",
        default=not ENVIRONMENT.session.compression_enabled,
...
def run_session(args: argparse.Namespace) -> int:
    ...
    capture_profile = collection.capture_profile
    request = collection.request(args.output_root, args.station_config)
...
def run_dagger_shadow(args: argparse.Namespace) -> int:
    from arx5_collection.collection.dagger.application import DaggerApplicationBuilder
...
def run_dagger_takeover_dry_run(args: argparse.Namespace) -> int:
    ...
    return (
        DaggerApplicationBuilder().build_takeover_dry_run(_dagger_run_spec(args)).run()
    )
...
def _dagger_run_spec(args: argparse.Namespace):
    from arx5_collection.collection.dagger.application import DaggerRunSpec
