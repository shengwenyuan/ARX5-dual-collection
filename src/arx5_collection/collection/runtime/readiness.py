...
from arx5_collection.collection.capture import configured_stream_contract
from arx5_collection.common.specs import RUNTIME_INTERFACE_SPEC

STREAM_CONTRACT = configured_stream_contract()
EXPECTED_STREAMS = {
    stream_id: definition[0] for stream_id, definition in STREAM_CONTRACT.items()
}
EXPECTED_TOPIC_TYPES = {
    stream_id: definition[1] for stream_id, definition in STREAM_CONTRACT.items()
}
...
class ReadinessLedger:
    ...
    def observe(
        ...
        if stream_id not in STREAM_CONTRACT:
            return
        with self._lock:
    ...
    def check(
        ...
        expected_topic, expected_type = STREAM_CONTRACT[stream_id]
...
class RosReadinessGate:
    ...
    def start(self) -> None:
        ...
        self._node = rclpy.create_node(
            "production_readiness_gate", context=self._context
        )
        qos = QoSProfile(
    ...
    def require_ready(self, stream_ids: Iterable[str] = STREAM_CONTRACT) -> None:
    ...
    def results(self, stream_ids: Iterable[str]) -> tuple[CheckResult, ...]:
        ...
        now_s = self.clock()
        return tuple(
            self.ledger.check(stream_id, topic_types, now_s, self.heartbeat_timeout_s)
