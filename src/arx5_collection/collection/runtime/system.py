...
class UsbfsManager:
    def __init__(
        self,
        required_mb: int = ENVIRONMENT.system.usbfs_memory_mb,
        parameter_path: Path = ENVIRONMENT.paths.usbfs_parameter,
    ) -> None:
        if required_mb <= 0:
...
class Usb2CanResolver:
    def __init__(
        self,
        device_root: Path = ENVIRONMENT.paths.device_root,
...
class CanInterfaceManager:
    def __init__(
        ...
        log_dir: Path,
        runner: CommandRunner = default_runner,
        startup_timeout_s: float = ENVIRONMENT.system.can_startup_timeout_s,
    ) -> None:
        self.arm = arm
        ...
        self.process = ManagedProcess(
            ProcessSpec(
                ...
                argv=(
                    ...
                    "-c",
                    "-f",
                    f"-{ENVIRONMENT.system.can_speed}",
                    str(tty_path),
                    arm.can_interface,
                ),
...
class SystemBringup:
    def __init__(
        ...
        interface_factory: (
            Callable[[ArmConfig, Path, Path], CanInterfaceManager] | None
        ) = None,
    ) -> None:
    ...
    def check(self) -> tuple[CheckResult, ...]:
        return (
            self.usbfs.check(),
            *(interface.check() for interface in self.interfaces),
        )
    ...
    def stop(self) -> tuple[CheckResult, ...]:
        ...
        try:
            ...
            restore = CheckResult(
                "usbfs_restore", CheckPhase.SHUTDOWN, False, str(error)
            )
        result = CheckResult(
            "system_cleanup",
            CheckPhase.SHUTDOWN,
