...
from arx5_collection.collection.capture import CaptureProfile, metadata_extensions
from arx5_collection.collection.configuration import CollectionConfig
from arx5_collection.collection.episode.cli import run_episode_loop
from arx5_collection.collection.episode.models import EpisodeRequest
from arx5_collection.collection.runtime.checks import CheckResult
from arx5_collection.collection.runtime.config import load_configured_station
from arx5_collection.collection.runtime.lifecycle import termination_as_interrupt
from arx5_collection.collection.runtime.orchestrator import GIB, ProductionSession
from arx5_collection.collection.runtime.processes import CameraSnapshotConfig
from arx5_collection.collection.reset import ResetState
...
DAGGER_STREAM_STATUS_PERIOD_S = ENVIRONMENT.monitor.dagger_display_period_s
...
class DaggerSessionBuilder:
    ...
    def build(
        ...
        backend = RosbagRecordingBackend(additional_topics=additional_recording_topics)
        return ProductionSession(
            ...
            warning_sink=lambda message: print(f"WARNING {message}", file=self.stderr),
            fail_directory=ENVIRONMENT.session.dagger_fail_directory,
...
class ShadowApplication:
    ...
    def run(self) -> int:
        ...
        status_sink = lambda message: print(message, file=self.stderr, flush=True)
        with (
            termination_as_interrupt(),
            OpenPiDaggerTransport(
            ...
            JsonlShadowLog(log_dir / "dagger-shadow.jsonl") as shadow_log,
            self.session,
        ):
...
class TakeoverApplication:
    ...
    def run(self) -> int:
        ...
        with (
            ...
            RosAuthorityEventPublisher() as events,
        ):
            policy = AsyncPi05PolicyClient(
                session_id=self.spec.session_id,
                prompt=self.settings.prompt,
...
class DaggerApplicationBuilder:
    ...
    def _load(
        ...
        collection = CollectionConfig.load(spec.collection_config)
        settings = DaggerCollectorSettings.load(spec.policy_config)
        request = collection.request(spec.output_root, spec.station_config)
        return settings, request, collection.capture_profile
