...
class PedalDeviceResolver:
    def __init__(
        self,
        sysfs_root: Path = ENVIRONMENT.paths.hidraw_sysfs_root,
...
class PedalTrigger:
    def __init__(
        ...
        debounce_s: float = ENVIRONMENT.trigger.pedal_debounce_s,
        ...
        if len(self._events_by_fd) != 2:
            raise ValueError(
                "pedal trigger devices must have different file descriptors"
            )
        self._last_press = {
