...
with config_path("runner/viewer.toml").open("rb") as _viewer_stream:
    _VIEWER_CONFIG = tomllib.load(_viewer_stream)
if (
    set(_VIEWER_CONFIG)
    != {
        "schema_version",
    ...
    or _VIEWER_CONFIG["schema_version"] != 1
):
    raise ValueError("viewer runner config must use schema_version 1 and exact keys")

DEFAULT_DATASET = Path(str(_VIEWER_CONFIG["dataset"]))
STATIC_ROOT = Path(__file__).with_name("static")
CAMERA_LABELS = {
    str(key): str(value) for key, value in dict(_VIEWER_CONFIG["camera_labels"]).items()
}
ACTION_HORIZON = PI05_ARX5_SPEC.dataset.action_horizon
MOTOR_NAMES = PI05_ARX5_SPEC.dataset.motor_names
THUMBNAIL_WIDTH = int(_VIEWER_CONFIG["thumbnail_width"])
JPEG_QUALITY = int(_VIEWER_CONFIG["jpeg_quality"])
if not 1 <= int(_VIEWER_CONFIG["port"]) <= 65535:
    raise ValueError("viewer port must be from 1 through 65535")
if THUMBNAIL_WIDTH <= 0 or not 1 <= JPEG_QUALITY <= 31:
            ...
            "episodes": [
                {
                    **item,
                    ...
                    "thumbnail_url": endpoint(
                        "thumbnail", self.root, item["episode_index"]
                    ),
                }
...
def read_parquet_episode(path: Path) -> dict[str, list]:
    ...
    return {
        ...
        "state": [[float(value) for value in row["observation.state"]] for row in rows],
    }
...
def frame_thumbnail_path(
    ...
    identity = f"{video}:{video.stat().st_size}:{video.stat().st_mtime_ns}:{frame}"
    digest = hashlib.sha256(identity.encode()).hexdigest()
...
class ViewerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            self.send_json(
                {"error": f"thumbnail generation failed: {error}"},
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--host", default=str(_VIEWER_CONFIG["host"]))
    parser.add_argument("--port", type=int, default=int(_VIEWER_CONFIG["port"]))
    return parser
