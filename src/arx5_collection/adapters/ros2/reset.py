...
DEFAULT_HOME = ENVIRONMENT.reset.home
DEFAULT_ARMS = (
    ArmResetSpec(
        "left",
        TEACHING_ARM_PROFILE.left_input_topic,
        f"/{TEACHING_ARM_PROFILE.left_controller_name}/go_home",
        f"/{TEACHING_ARM_PROFILE.left_controller_name}/gravity_compensation",
    ),
    ArmResetSpec(
        "right",
        TEACHING_ARM_PROFILE.right_input_topic,
        f"/{TEACHING_ARM_PROFILE.right_controller_name}/go_home",
        f"/{TEACHING_ARM_PROFILE.right_controller_name}/gravity_compensation",
    ),
)
...
class RosDualArmResetController:
    ...
    def __init__(
        ...
        timeout_s: float = ENVIRONMENT.reset.timeout_s,
        position_tolerance_rad: float = ENVIRONMENT.reset.position_tolerance_rad,
        velocity_tolerance_rad_s: float = ENVIRONMENT.reset.velocity_tolerance_rad_s,
        stable_s: float = ENVIRONMENT.reset.stable_s,
        ...
        if (
            min(timeout_s, position_tolerance_rad, velocity_tolerance_rad_s, stable_s)
            <= 0
        ):
    ...
    def sample_positions(self) -> dict[str, tuple[float, ...]]:
        with self._samples_lock:
            return {name: sample[0] for name, sample in self._samples.items()}
    ...
    def _wait_for_convergence(self, command_started_s: float) -> None:
        ...
        while self.clock() < deadline:
            ...
            converged = len(current) == len(self.arms) and all(
                ...
                and all(
                    abs(value) <= self.velocity_tolerance_rad_s for value in velocity
                )
