...
class BucketLinkSpec:
    ...
    def validate(self) -> None:
        ...
        if self.throughput_limit_bytes not in PFS_THROUGHPUT_LIMITS:
            raise ValueError(
                "bucketlink.throughput_limit_bytes is not a supported PFS limit"
            )
...
class BaiduBucketLinkClient:
    ...
    def from_default_credentials(cls, endpoint: str) -> BaiduBucketLinkClient:
        try:
            ...
            from baiducloud_python_sdk_core.bce_client_configuration import (
    ...
    while True:
        ...
        if status.status == 2:
            ...
            if status.destination != spec.pfs_path:
                raise RuntimeError(
                    "BucketLink destination differs from the frozen request"
                )
            return status
...
def validate_report(text: str) -> dict[str, int]:
    ...
    if counts["failedCount"]:
        raise RuntimeError(
            f"BucketLink report contains {counts['failedCount']} failed files"
        )
    if counts["skippedCount"]:
        raise RuntimeError(
            f"BucketLink report contains {counts['skippedCount']} skipped files"
        )
...
def _load_bce_credentials(bcecmd_path: Path | None = None) -> tuple[str, str]:
    ...
    if not access_key or not secret_key:
        raise RuntimeError(
            f"bcecmd credentials file has no complete AK/SK pair: {path}"
        )
    return access_key, secret_key
