...
from .bos2pfs import BaiduBucketLinkClient
from .bos2pfs import BucketLinkClient, BucketLinkSpec, BucketLinkStatus
from .bos2pfs import mounted_report_path, validate_report, wait_for_bucket_link
from .streaming_conversion.alignment import require_enter_confirmation
from .streaming_conversion.application import StreamingApplicationResult
from .streaming_conversion.application import StreamingRunRequest
from .streaming_conversion.application import execute_streaming_config
from .streaming_conversion.config import BufferedRuntimeConfig
from .streaming_conversion.config import PrefetchRuntimeConfig
from .streaming_conversion.config import StreamingConversionConfig
...
def execute_bucketlink_conversion(
    ...
    *,
    ...
    output_stream.write(json.dumps({"bucketlink_report": report}, sort_keys=True) + "\n")
    output_stream.flush()

    downstream = StreamingRunRequest(
...
def load_bucketlink_config(
    path: Path,
) -> tuple[BucketLinkSpec, StreamingConversionConfig]:
    ...
    if not isinstance(runtime, dict) or set(runtime) != runtime_keys:
        raise ValueError(f"BucketLink runtime keys must be exactly {sorted(runtime_keys)}")
    ...
    conversion = StreamingConversionConfig.from_mapping(payload)
    _validate_target_mapping(spec, conversion)
    return spec, conversion


def _validate_target_mapping(
    spec: BucketLinkSpec, config: StreamingConversionConfig
) -> None:
    ...
    if len(config.source.include_paths) != 1:
        raise ValueError("one BucketLink batch requires exactly one source.include_paths entry")
    ...
    if mounted_target.resolve(strict=False) != include_target.resolve(strict=False):
        raise ValueError("bucketlink.mounted_path must equal the configured source include path")
    if runtime.pfs_root.resolve(strict=False) not in mounted_target.resolve(strict=False).parents:
        raise ValueError("bucketlink.mounted_path must be below runtime.pfs_root")


def _prepare_empty_target(spec: BucketLinkSpec, config: StreamingConversionConfig) -> None:
...
def _render_progress(status: BucketLinkStatus, output_stream: TextIO) -> None:
    output_stream.write(
        json.dumps(
            {"bucketlink_progress": {"status": status.status, "progress": status.progress}},
            sort_keys=True,
        )
        + "\n"
    )
...
def _conversion_slot(
    config: StreamingConversionConfig, output_stream: TextIO
) -> Iterator[None]:
    runtime = config.runtime
