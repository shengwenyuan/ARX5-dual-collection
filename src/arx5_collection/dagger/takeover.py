...
class AuthorityTimeline:
    ...
    def start_episode(self, control_epoch: int) -> None:
        ...
        self._intervention_count = 0
        self._segments = []
        self._active = (ControlOwner.MODEL, 0.0, None)
        self._emit(
            AuthorityEventType.POLICY_ACTIVE,
            intervention_id=0,
            control_epoch=control_epoch,
            reason="episode_started_dry_run",
        )
    ...
    def policy_active(self, intervention_id: int, control_epoch: int) -> None:
        at_s = self._offset_s()
        ...
        self._emit(
            ...
            intervention_id=intervention_id,
            control_epoch=control_epoch,
            reason="no_action_gateway_ready",
        )
...
class TakeoverController:
    ...
    def start_episode(self, episode_id: str) -> None:
        ...
        self.status_sink("DAGGER DRY-RUN MODEL_CONTROL: no action output")
    ...
    def _resume(self) -> None:
        ...
        try:
            ...
            self.state = TakeoverState.MODEL_CONTROL
            self.status_sink(
                f"DAGGER DRY-RUN POLICY_ACTIVE intervention={self.intervention_id} "
                f"epoch={self.control_epoch}; no action output"
            )
        except Exception as error:
            self._fault(error)
