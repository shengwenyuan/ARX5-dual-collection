...
def select_causal_step(
    camera_left: tuple[YuyvFrame, ...],
    camera_overview: tuple[YuyvFrame, ...],
    camera_right: tuple[YuyvFrame, ...],
    left_arm: tuple[RawArmSample, ...],
    right_arm: tuple[RawArmSample, ...],
    now_ns: int,
    constraints: ObservationConstraints = ObservationConstraints(),
) -> VlaObservationStep:
    buffers = {
    ...
    missing = tuple(name for name, values in buffers.items() if not values)
    if missing:
    ...
    fresh_groups: list[tuple[YuyvFrame, YuyvFrame, YuyvFrame, int, int]] = []
    newest_snapshot_age_ns: int | None = None
    for overview in reversed(camera_overview):
        left = min(camera_left, key=lambda frame: abs(frame.stamp_ns - overview.stamp_ns))
        right = min(
            camera_right, key=lambda frame: abs(frame.stamp_ns - overview.stamp_ns)
        )
        camera_stamps = (left.stamp_ns, overview.stamp_ns, right.stamp_ns)
        cutoff_ns = max(camera_stamps)
        age_ns = now_ns - cutoff_ns
        if newest_snapshot_age_ns is None:
            newest_snapshot_age_ns = age_ns
        if age_ns < 0 or age_ns > constraints.max_snapshot_age_ns:
            continue
        camera_span_ns = max(camera_stamps) - min(camera_stamps)
        fresh_groups.append((left, overview, right, cutoff_ns, camera_span_ns))

    if not fresh_groups:
        raise ObservationUnavailableError(
            ObservationFailureCode.SNAPSHOT_STALE,
            observed_ns=newest_snapshot_age_ns,
            limit_ns=constraints.max_snapshot_age_ns,
        )

    aligned_groups = [
        group
        for group in fresh_groups
        if group[4] <= constraints.max_camera_span_ns
    ]
    if not aligned_groups:
        raise ObservationUnavailableError(
            ObservationFailureCode.CAMERA_SPAN_EXCEEDED,
            observed_ns=min(group[4] for group in fresh_groups),
            limit_ns=constraints.max_camera_span_ns,
        )

    arm_failure: ObservationUnavailableError | None = None
    for left, overview, right, cutoff_ns, _ in aligned_groups:
        left_state = _latest_not_after(left_arm, cutoff_ns)
        right_state = _latest_not_after(right_arm, cutoff_ns)
        if left_state is None:
            arm_failure = arm_failure or ObservationUnavailableError(
                ObservationFailureCode.LEFT_ARM_STALE,
                limit_ns=constraints.max_arm_age_ns,
                detail="no state at or before camera cutoff",
            )
            continue
        left_age_ns = cutoff_ns - left_state.stamp_ns
        if left_age_ns > constraints.max_arm_age_ns:
            arm_failure = arm_failure or ObservationUnavailableError(
                ObservationFailureCode.LEFT_ARM_STALE,
                observed_ns=left_age_ns,
                limit_ns=constraints.max_arm_age_ns,
            )
            continue
        if right_state is None:
            ...
            continue
        right_age_ns = cutoff_ns - right_state.stamp_ns
        if right_age_ns > constraints.max_arm_age_ns:
            arm_failure = arm_failure or ObservationUnavailableError(
                ObservationFailureCode.RIGHT_ARM_STALE,
                observed_ns=right_age_ns,
                limit_ns=constraints.max_arm_age_ns,
            )
            continue
    ...
    assert arm_failure is not None
    raise arm_failure


def _latest_not_after(
    samples: tuple[RawArmSample, ...], cutoff_ns: int
) -> RawArmSample | None:
    return next(
        (sample for sample in reversed(samples) if sample.stamp_ns <= cutoff_ns),
        None,
    )
