...
MONOTONIC_ANCHOR_TOLERANCE_NS = 1_000
BAG_ANCHOR_TOLERANCE_NS = 2_000_000
...
def _metadata_contract(
    ...
    for index, raw in enumerate(raw_segments):
        ...
        intervention_id = (
            int(segment["intervention_id"])
            if "intervention_id" in segment
            else None
        )
        ...
        if state == "resume":
            if event.event_type is AuthorityEventType.POLICY_ACTIVE:
                ...
                if segment_index >= len(metadata_segments):
                    raise ValueError("POLICY_ACTIVE has no matching model metadata segment")
                active = metadata_segments[segment_index]
                if active.owner != "model":
                    raise ValueError("POLICY_ACTIVE does not match a model metadata segment")
            ...
            if event.event_type is AuthorityEventType.FAULT_HOLD:
                state = "fault"
                continue
            raise ValueError(f"unexpected {event.event_type.name} while policy is pending")
        if state == "policy":
            if event.event_type is AuthorityEventType.TAKEOVER_REQUESTED:
                assert active is not None
                expected_intervention += 1
                if event.intervention_id != expected_intervention:
                    raise ValueError("TAKEOVER_REQUESTED intervention id is not consecutive")
            ...
            if event.event_type is AuthorityEventType.FAULT_HOLD:
                ...
                state = "fault"
                continue
            raise ValueError(f"unexpected {event.event_type.name} during policy control")
        if state == "handover":
            if event.event_type is AuthorityEventType.HUMAN_ACTIVE:
                ...
                active = metadata_segments[segment_index]
                if active.owner != "human" or active.intervention_id != expected_intervention:
                    raise ValueError("HUMAN_ACTIVE does not match human metadata segment")
                candidates.append(event.monotonic_time_ns - active.started_offset_ns)
                state = "human"
                continue
            if event.event_type is AuthorityEventType.FAULT_HOLD:
...
def classify_authority(
    ...
    try:
        episode_id, duration_ns, intervention_count, metadata_segments = _metadata_contract(metadata)
        ...
        if anchor_spread > MONOTONIC_ANCHOR_TOLERANCE_NS:
            raise ValueError(
                f"metadata/event monotonic anchor spread {anchor_spread} ns exceeds "
                f"{MONOTONIC_ANCHOR_TOLERANCE_NS} ns"
            )
        ...
        if bag_spread > BAG_ANCHOR_TOLERANCE_NS:
            raise ValueError(
                f"authority bag anchor spread {bag_spread} ns exceeds "
                f"{BAG_ANCHOR_TOLERANCE_NS} ns"
            )
