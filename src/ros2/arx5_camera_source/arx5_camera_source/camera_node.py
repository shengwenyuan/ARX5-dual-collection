...
class D405Source(Node):
    def __init__(self) -> None:
        ...
        self.width = int(self.declare_parameter("width", 0).value)
        self.height = int(self.declare_parameter("height", 0).value)
        self.fps = int(self.declare_parameter("fps", 0).value)
        self.frame_timeout_ms = int(self.declare_parameter("frame_timeout_ms", 0).value)
        self.status_period_s = float(
            self.declare_parameter("stream_status_period_s", 0.0).value
        )
        self.reliability = str(self.declare_parameter("reliability", "").value)
        self.color_topic = str(self.declare_parameter("color_topic", "").value)
        self.depth_topic = str(self.declare_parameter("depth_topic", "").value)
        self.stream_status_topic = str(
            self.declare_parameter("stream_status_topic", "").value
        )
        ...
        self.color_publisher = self.create_publisher(Image, self.color_topic, qos)
        ...
        self.status_publisher = self.create_publisher(
            StreamStatus, self.stream_status_topic, status_qos
        )
    ...
    def _validate_parameters(self) -> None:
        ...
        for name, topic in (
            ...
            ("stream_status_topic", self.stream_status_topic),
        ):
        ...
        for sensor in device.query_sensors():
            if sensor.supports(rs.option.global_time_enabled):
                if sensor.get_option(rs.option.global_time_enabled) != 1.0:
                    raise RuntimeError(
                        f"RealSense {self.serial} does not enable Global Time"
                    )
                supported += 1
        if supported == 0:
    ...
    def publish_once(self) -> None:
        ...
        if not color_frame or not depth_frame:
            raise RuntimeError(
                f"RealSense {self.serial} returned an incomplete frameset"
            )
