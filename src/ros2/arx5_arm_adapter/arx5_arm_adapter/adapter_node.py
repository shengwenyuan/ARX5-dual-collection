...
ARM_TOPICS = (
    ("left", "left_input_topic", "left_output_topic"),
    ("right", "right_input_topic", "right_output_topic"),
)
...
class ArmStateAdapter(Node):
    def __init__(self) -> None:
        ...
        self._status_publisher = self.create_publisher(
            StreamStatus,
            self._required_topic("stream_status_topic"),
            status_qos,
        )
        self._arm_publishers = {}
        self._arm_subscriptions = []
        self._status_reporters = {}
        for arm, input_parameter, output_parameter in ARM_TOPICS:
            input_topic = self._required_topic(input_parameter)
            output_topic = self._required_topic(output_parameter)
            ...
            self._arm_subscriptions.append(
                self.create_subscription(RobotStatus, input_topic, callback, data_qos)
            )
    ...
    def _required_topic(self, name: str) -> str:
        topic = str(self.declare_parameter(name, "").value)
    ...
    def _callback_for(
        ...
        def callback(source: RobotStatus) -> None:
            ...
            message_stamp_ns = int(source.header.stamp.sec) * 1_000_000_000 + int(
