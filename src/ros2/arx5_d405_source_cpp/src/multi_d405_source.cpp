...
{
  CameraEndpoint(
    rclcpp::Node & node, const std::string & role,
    const std::string & color_topic, const std::string & depth_topic)
  : role(role),
...
public:
  ...
  : Node("multi_d405_source"),
    width_(static_cast<int>(positive_size_parameter("width", 0))),
    height_(static_cast<int>(positive_size_parameter("height", 0))),
    fps_(static_cast<int>(positive_size_parameter("fps", 0))),
    snapshot_enabled_(declare_parameter<bool>("enable_snapshot_ipc", false)),
    buffer_(positive_size_parameter("camera_history_size", 0),
      positive_size_parameter("arm_history_size", 0)),
    constraints_{
    ...
    status_publisher_(create_publisher<StreamStatus>(
      required_string("stream_status_topic"), rclcpp::QoS(32).reliable()))
  {
    ...
    endpoints_.push_back(std::make_unique<CameraEndpoint>(
      *this, "left", required_string("color_topic_left"),
      required_string("depth_topic_left")));
    endpoints_.push_back(std::make_unique<CameraEndpoint>(
      *this, "overview", required_string("color_topic_overview"),
      required_string("depth_topic_overview")));
    endpoints_.push_back(std::make_unique<CameraEndpoint>(
      *this, "right", required_string("color_topic_right"),
      required_string("depth_topic_right")));
    ...
    status_timer_ = create_wall_timer(
      std::chrono::nanoseconds(
        positive_milliseconds_parameter("stream_status_period_ms", 0.0)),
      [this]() {publish_status();});
...
private:
  ...
  void configure_snapshot_endpoint(const std::string & socket_path)
  {
    left_arm_subscription_ = arm_subscription<ArmState>(
      required_string("left_arm_topic"),
    ...
    right_arm_subscription_ = arm_subscription<ArmState>(
      required_string("right_arm_topic"),
