...
CAMERA_STREAMS = tuple(
    stream for stream in RGBD_STREAMS if stream.id.startswith("camera_")
)
ARM_STREAMS = tuple(
    stream for stream in RGBD_STREAMS if stream.id.endswith("_arm_state")
)
PROFILES = {
...
STATUS_TOPIC = RUNTIME_INTERFACE_SPEC.stream_status_topic
STATUS_TYPE = RUNTIME_INTERFACE_SPEC.stream_status_message_type
...
def analyze(
    ...
    return {
        ...
        "rgbd_pairing": ({} if telemetry_only else rgbd_pairing(header_stamps)),
        "telemetry": {
