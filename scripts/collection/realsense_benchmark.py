...
from arx5_collection.common.metrics import timing_summary
    ...
    try:
        ...
        for serial, process in zip(serials, processes, strict=True):
            if serial not in reports:
                reason = (
                    "worker timeout"
                    if process in timed_out
                    else f"worker exit code {process.exitcode}"
                )
                reports[serial] = {"serial": serial, "error": reason}

        result["cameras"] = [reports[serial] for serial in serials]
        errors = [
            report.get("error") for report in result["cameras"] if report.get("error")
        ]
        if errors:
    ...
    try:
        ...
        report = {
            ...
            **timing_summary(timestamps_ns),
            ...
            "first_device_timestamp_ms": (
                device_timestamps_ms[0] if device_timestamps_ms else None
            ),
            "last_device_timestamp_ms": (
                device_timestamps_ms[-1] if device_timestamps_ms else None
            ),
