...
from arx5_collection.dataset_pipeline.application import (
...
from arx5_collection.dataset_pipeline.execution.coordinator import (
    StageWork,
    stage_episode,
)
from arx5_collection.dataset_pipeline.persistence.manifest import RunManifest
from arx5_collection.dataset_pipeline.execution.models import (
    DiscoveryResult,
    EpisodeCandidate,
    FileIdentity,
    SelectionEntry,
...
def _workers(value: str) -> list[int]:
    ...
    if not result or any(item < 1 for item in result):
        raise argparse.ArgumentTypeError(
            "workers must be comma-separated positive ints"
        )
    return result
...
def run_stage(args: argparse.Namespace) -> int:
    ...
    for workers in args.workers:
        ...
        metric_path = benchmark_root / "results" / f"{campaign}-stage-w{workers}.json"
...
def run_e2e(args: argparse.Namespace) -> int:
    ...
    config = DatasetPipelineConfig.load(source_config_path)
    ...
    recipe = load_pipeline_recipe(source_config_path, config)
    ...
    metric = {
        ...
        "frames_s": (
            result.snapshot.frame_count / elapsed if result is not None else 0.0
        ),
