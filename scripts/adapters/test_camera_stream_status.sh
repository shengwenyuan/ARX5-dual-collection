...
status_topic=$(python3 -c 'from arx5_collection.common.specs import RUNTIME_INTERFACE_SPEC; print(RUNTIME_INTERFACE_SPEC.stream_status_topic)')
...
setsid /opt/arx5-dual-collection/scripts/adapters/run_d405_sources.sh "${station_config}" \
  >"${output_bag}.camera.log" 2>&1 &
camera_pid=$!

for _ in $(seq 1 100); do
  publishers=$(ros2 topic info "${status_topic}" 2>/dev/null \
...
setsid ros2 bag record --storage mcap --output "${output_bag}" --topics \
  "${status_topic}" >"${output_bag}.record.log" 2>&1 &
recorder_pid=$!

for _ in $(seq 1 50); do
  subscribers=$(ros2 topic info "${status_topic}" 2>/dev/null \
