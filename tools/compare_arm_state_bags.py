...
TOPIC_PAIRS = (
    ("/arm_master_l_status", "/embodiments/left_arm/state"),
    ("/arm_master_r_status", "/embodiments/right_arm/state"),
)
...
def _compare_topic(input_bag: Path, output_bag: Path, source: str, logical: str) -> dict[str, Any]:
    sentinel = object()
    count = 0
    ...
    for index, (source_message, logical_message) in enumerate(
        zip_longest(_messages(input_bag, source), _messages(output_bag, logical), fillvalue=sentinel)
    ):
        if source_message is sentinel or logical_message is sentinel:
            raise RuntimeError(f"message count differs for {source} -> {logical} at index {index}")
        if _header(source_message) != _header(logical_message):
            raise RuntimeError(f"Header differs for {source} -> {logical} at index {index}")
        if _source_values(source_message) != _logical_values(logical_message):
            raise RuntimeError(f"fields differ for {source} -> {logical} at index {index}")
