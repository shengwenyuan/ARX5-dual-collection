...
- Status: `in-progress`（等待两颗 D405 改接 USB3）
- Target: `w3-arx5`
...
- 确认左右机械臂 CAN 映射，只读取状态，不发送任何运动命令。
...
3. 识别机械臂相关 CAN 接口和现有 SDK；仅调用无运动副作用的状态读取入口。
...
- 双臂只调用构造、状态 Getter 和 `close`。两秒测试完成 121 次读取，约 62.5 Hz，最大间隔 16.81 ms，无 fault、丢包或总线错误。
