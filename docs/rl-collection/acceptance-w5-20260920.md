...
同日完成 [A/B/C 传输实验](command-transport-experiment-20260920.md)：A 跨局复用 publisher、B 增加匹配等待，均在第 14–20 局复现缺 step 0 和约 5.1 秒积压；C 每局重建现有 publisher 生命周期并等待匹配，20/20 局、8,000 条全部完整。异常局发送前已匹配，因此仅补匹配等待无效。每局重建为已通过本轮合成实验的候选修复，尚未修改生产代码；DDS 内部根因与真机复测仍未完成。

第二轮新增 [160 局无真机定位](command-transport-root-cause-20260920.md)：累计序号达到接收端 5,000 样本上限后，新 reader 把此前序号计为未知缺失并拒绝首条，Fast DDS 2.14.6 进程内可靠交付未及时补回；原真机第 14 局开始前累计 5,194 条，与该机制一致。仅每局重建 publisher（保留 node/context）20/20 局完整；显式关闭进程内交付也 20/20 完整。已定位原因但尚未修改生产实现，完整验收仍不通过。

后续修复已实现：infer 与 DAgger 共用按局 writer 管理，两话题各 20/20 局、8,000 条完整通过真实 ROS/MCAP 无真机回归，见 [修复验收](publisher-lifecycle-fix-20260920.md)。已部署 w5，用户已追加录制 10 局；动作回读及完整真机验收仍待完成。随后按用户指令删除本批 6 局异常，删除前记录保留为历史证据，当前两批保留数据统计见 [修复验收](publisher-lifecycle-fix-20260920.md)。

## 剩余验收

1. publisher 修复已通过无真机回归并部署；继续回读新批次验证首条命令缺失，要求连续多局每局从 step 0 起、MCAP 计数与发送计数一致；复用现有 Recorder 与采集模式，不扩张为完整追踪系统。
2. 用户明确本批只按正常流程操作：长按/释放、录制中 Ctrl+C、双踏板冲突均未验证，保持待验收。现有离线测试不能代替这些现场行为。
3. exporter 的 fail 接纳、关键缺失整局隔离、动作对齐和 replay 联调仍单独执行；保留数据用于后续成功、正常失败联调；已删除异常局的历史诊断证据见归档。
