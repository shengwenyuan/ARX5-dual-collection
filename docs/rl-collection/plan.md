...
更新：2026-09-20 · v0.5。状态：用户已完成 w5 真机 19 局，正常多局操作已确认；6 局缺首条动作记录，完整验收暂不通过，完成版本仍未锁定。
...
| 项目 | 当前状态 |
| --- | --- |
| 计划 | v0.5，保持既有三方边界与 DAgger/infer 共用策略运行层 |
...
| C4 | 已只读验收真机 19 局：13 局通过输出契约检查，6 局缺 step 0，阻塞完整通过；长按/释放、录制中 Ctrl+C、双踏板冲突未测 |
...
- v0.5：记录 w5 真机 19 局的验收结果和 6 局缺首动作的阻塞项；普通 fail 与数据质量失败分开，保留剩余现场验收。
