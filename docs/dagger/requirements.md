...
- Status: `aligned`
- Updated: 2026-09-18
- Scope: 本地 VLA 推理、显式人工接管、连续 MCAP 与控制权事件记录
...
本仓库只记录连续 MCAP、稀疏 authority 事件及 metadata 控制区间，不提供离线分类、片段裁切、训练样本筛选或 LeRobot 导出。

外部处理系统可根据 `HUMAN_ACTIVE -> RESUME_REQUESTED` 识别人工纠正区间；交接、重新推理、模型控制和故障区间不能误标为专家操作。原始事件时间、intervention ID 和控制区间继续保留，外部系统负责验证与训练数据组织。
