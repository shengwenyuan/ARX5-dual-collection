...
实时逐项校验：角点完整、覆盖率（默认 ≥1.5%）、清晰度（≥40）、边缘距离（≥8 px）、相机时钟、图像时效；左右臂分别显示反馈时效（≤100 ms）、关节限位和静止速度（≤0.04 rad/s）；双臂连续停稳（≥0.6 s）、停稳后曝光、与已有位姿的差异、非示教臂是否保持原位、相邻运动段时长。某项不通过不影响其他项显示绿色。
