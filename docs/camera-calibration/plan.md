...
普通棋盘有对称歧义。首次以画面 A 对应的外侧内角点作为物理标记；程序按连续视图匹配编号，按 Space 仍意味着用户确认 A、+X/+Y 与同一物理编号一致。丢失棋盘后重新出现、大幅转动时不能保证自动识别物理 A；编号不符时先返回原视角恢复一致，再保存。回放按该站示教图匹配编号，明显偏离则拒绝；不把普通棋盘当作自带唯一 ID。

实时逐项校验：角点完整、覆盖率（默认 ≥1.5%）、清晰度（≥40）、边缘距离（≥8 px）、相机时钟、图像时效；左右臂分别显示反馈时效（≤100 ms）、关节限位和静止速度（≤0.01 rad/s）；双臂连续停稳（≥0.6 s）、停稳后曝光、与已有位姿的差异、非示教臂是否保持原位、相邻运动段时长。某项不通过不影响其他项显示绿色。
