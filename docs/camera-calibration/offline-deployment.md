...
标定与 collection 均使用原生 848×480@30 RGB8；旧 720p 路线需重新示教。
首次输入内角点列、行、单格 mm；画面检测有效且停稳后按 **Space** 保存实测关节角。
每项校验独立显示 PASS / WAIT；Backspace 删除上一条。棋盘编号在离线计算时统一，不携带原工作台像素。
建议每路 35 个以上独立候选姿态，覆盖不同方向、倾角和距离；关闭窗口自动验证，合格即完成，否则保留草稿。退出按终端提示手动归位。

默认文件为 `/var/lib/arx5-collection/calibration/routes/<role>.json`。
每次 Space / Backspace 原子落盘；关闭窗口并验证通过后才允许自动回放。再次 `--teach` 是新示教，
旧文件进入 `routes/history`。后续使用 `--record` 遍历全部候选、跳过视觉无效点，并在归位退出硬件后自动计算；`--solve` 用于离线重算。
路径 JSON 不绑定工作台，本站身份/棋盘/安装批次记录在新 run；至少需要 20 个有效点（15 训练 + 5 验证）。

自动回放默认速度上限 0.10 rad/s，先自动将双臂依次移到 HOME 并停稳，再到路线首姿。无需人工精确摆位；HOME 使用标定六关节轨迹并保留夹爪目标。
