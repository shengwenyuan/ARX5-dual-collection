...
- 三路摄像机：848×480 RGB8 彩色图像、同分辨率且与彩色图像对齐的 `16UC1` Depth，目标频率 30 Hz。
...
- 禁止插值、补帧、重复帧和伪造同步帧。
- D405 官方规格明确不支持多机硬件同步信号。`librealsense v2.54.2` 的官方多机示例为每台设备建立独立 Pipeline，不提供三台 D405 跨设备同步保证。
- v0.1 使用各设备的 Global Time 时间戳独立记录，不跨相机配对、等待、丢帧或重组 frameset；只做单机彩色与 Depth 的空间对齐。
...
- 实时监督使用 Source/Adapter 发布的轻量状态，不重复订阅六路大图；最终 JSON 指标以 MCAP 落盘审计为准。
...
v0.1 每颗 D405 只发布彩色图与对齐 Depth，不发布 `CameraInfo`。设备身份、内参版本和标定引用属于 metadata；跨相机外参标定流程留到 v0.2。
