...
CONFIG = (
    ROOT.joinpath("config/collection/dagger.policy.example.toml")
    .read_text()
    .replace("/checkpoints/task_checkpoint/9999", "/checkpoints/example/9999")
    .replace("0" * 64, "a" * 64)
    .replace('prompt = "Describe the task"', 'prompt = "Stacking paper cups"')
    .replace("inference_timeout_s = 30.0", "inference_timeout_s = 12.5")
)
...
class DaggerConfigTest(unittest.TestCase):
    ...
    def test_model_execution_parameters_are_configurable(self) -> None:
        custom = (
            CONFIG.replace("action_horizon = 50", "action_horizon = 12")
            .replace(
                "sequential_execution_steps = 10",
                "sequential_execution_steps = 4",
            )
            .replace("control_rate_hz = 25.0", "control_rate_hz = 20.0")
            .replace("rate_hz = 25.0", "rate_hz = 20.0")
        )
    ...
    def test_v3_rtc_numbers_are_loaded_from_one_typed_profile(self) -> None:
        ...
        path = root / "config" / "collection" / "dagger.rtc.example.toml"
        ...
        self.assertEqual(
            collector.rtc_rollout.safe_window_steps(collector.checkpoint_profile),
            19,
        )

    def test_rtc_profile_accepts_configured_gripper_offset(self) -> None:
        root = Path(__file__).resolve().parents[3]
        payload = (root / "config/collection/dagger.rtc.example.toml").read_text()
        self.path.write_text(
            payload.replace("control_rate_hz = 25.0", "control_rate_hz = 30.0")
            .replace("policy_wait_timeout_s = 0.35", "policy_wait_timeout_s = 0.28")
            .replace("normalized_action_offset = 0.0", "normalized_action_offset = 0.1")
            .replace("max_normalized_gripper = 1.0", "max_normalized_gripper = 1.11")
            .replace(
                "0" * 64,
    ...
    def test_rtc_snapshot_timeout_must_precede_request_deadline(self) -> None:
        root = Path(__file__).resolve().parents[3]
        payload = (
            root / "config" / "collection" / "dagger.rtc.example.toml"
        ).read_text()
        self.path.write_text(
