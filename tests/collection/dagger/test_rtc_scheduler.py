...
from arx5_collection.collection.dagger.action_gateway import (
    DualArmJointState,
    JointActionSafety,
...
from arx5_collection.collection.dagger.rtc_scheduler import (
...
def ticket(
    ...
    actions = (action(),) * fixed_prefix + (action(value, right_gripper),) * (
        8 - fixed_prefix
    )
...
class RtcSchedulerTest(unittest.TestCase):
    ...
    def test_records_gripper_saturation_without_faulting(self) -> None:
        ...
        saturated = [row for row in diagnostics if row["event"] == "gripper_saturated"]
