...
from arx5_collection.collection.runtime.cli import build_parser, load_configured_station
...
class RosCommandSetTest(unittest.TestCase):
    ...
    def test_arx_uses_frozen_official_v2_launch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            dagger = RosCommandSet(Path(directory)).arx5_controller(DAGGER_ARM_PROFILE)
    ...
    def test_arm_adapter_profile_changes_only_vendor_input_topics(self) -> None:
        ...
        self.assertIn("left_input_topic:=/arm_master_l_status", teaching.spec.argv)
        self.assertIn("right_input_topic:=/arm_master_r_status", teaching.spec.argv)
        self.assertIn("left_input_topic:=/arm_slave_l_status", dagger.spec.argv)
        self.assertIn("right_input_topic:=/arm_slave_r_status", dagger.spec.argv)
        self.assertIn(
            "left_output_topic:=/embodiments/left_arm/state", dagger.spec.argv
        )
        self.assertIn(
            "right_output_topic:=/embodiments/right_arm/state", dagger.spec.argv
        )
