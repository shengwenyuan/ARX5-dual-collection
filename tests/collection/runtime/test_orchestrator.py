...
from arx5_collection.collection.runtime.orchestrator import ProductionSession
from arx5_collection.collection.runtime.processes import ProcessExit, ProcessSpec
...
class ProductionSessionTest(unittest.TestCase):
    def test_rgb_only_waits_for_only_five_required_streams(self) -> None:
        ...
        require = next(event for event in events if event.startswith("gate:require:"))
    ...
    def test_one_session_starts_sources_once_and_stops_in_reverse(self) -> None:
        ...
        self.assertLess(
            events.index("home:close"), events.index("ros:stop:d405-source")
        )
