...
from arx5_collection.collection.runtime.triggers import AutoTriggerFactory
from arx5_collection.collection.runtime.cli import DEFAULT_STATION_CONFIG, build_parser
...
class AutoTriggerFactoryTest(unittest.TestCase):
    ...
    def test_production_cli_has_only_auto_and_defaults_station_path(self) -> None:
        ...
        self.assertEqual(
            run.station_config, Path("/var/lib/arx5-collection/station.json")
        )
        self.assertEqual(run.output_root, Path("/reports/episodes"))
        self.assertEqual(run.collection_config, Path("collection.toml"))
        ...
        self.assertEqual(checkpoint_sha.checkpoint, Path("/checkpoints/example/9999"))
