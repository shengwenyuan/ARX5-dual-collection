...
from arx5_collection.collection.episode.models import EpisodeBlocked, StreamMetrics
from arx5_collection.collection.episode.ports import TriggerEvent, TriggerSignal
...
class EpisodeCliTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.collection_config = ROOT / "config/collection/fold-cloth-rgbd.toml"
    ...
    def test_a_aborts_current_episode_and_continues(self) -> None:
        trigger = ContextTrigger([True, TriggerEvent.ABORT, True, True])
        output = io.StringIO()
        exit_code = run_cli(
            runtime_factory=self.runtime_factory(),
            argv=self.argv(2),
    ...
    def request(self):
        return CollectionConfig.load(self.collection_config).request(
            self.output_root,
            STATION_PATH,
        )
