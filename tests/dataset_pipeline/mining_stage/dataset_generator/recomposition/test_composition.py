...
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.models import (
    CompositionConfig,
)
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.models import (
    CompositionPlan,
...
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.planner import (
    _validate_v3_shard_selection,
)
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.planner import (
    build_plan,
)
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v21 import (
    build_v21,
)
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v21 import (
    _rewrite_parquet,
)
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v3_worker import (
    run as run_v3_worker,
)
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v3_worker import (
    _ordered_whole_shard_groups,
)
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v3 import (
    build_v3,
)
...
class CompositionTest(unittest.TestCase):
    ...
    def test_manifest_order_is_frozen_and_v21_build_is_standalone(self) -> None:
        ...
        def rewrite(
            source, target, _local, _global, _offset, local_tasks, global_tasks
        ):
        ...
        with patch(
            "arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v21._rewrite_parquet",
            side_effect=rewrite,
        ):
            result = build_v21(
                plan,
        ...
        self.assertEqual(
            [row["lerobot_episode_index"] for row in output_rows], [0, 1, 2]
        )
        ...
        self.assertEqual(
            _read(result.output_path / "snapshot.json")["status"], "committed"
        )
    ...
    def test_one_snapshot_may_preserve_multiple_episode_task_descriptions(self) -> None:
        ...
        config = self.root / "composition.toml"
        config.write_text(
            _toml(self.root / "output", [("multi", source, "select_all = true")])
        )
        ...
        self.assertEqual(
            [item.episode.tasks for item in plan.selected], [("task A",), ("task B",)]
        )

    def test_materialization_failure_preserves_diagnostic_staging_without_commit_marker(
        self,
    ) -> None:
        source = self._snapshot("source-a", ("segment-a0",), "task")
        config = self.root / "composition.toml"
    ...
    def test_v3_worker_splits_complete_shard_components_in_recipe_order(self) -> None:
        ...
        self.assertEqual(
            _ordered_whole_shard_groups(Metadata(), [2, 3, 0, 1]), [[2, 3], [0, 1]]
        )
    ...
    def test_v3_worker_refuses_unpinned_runtime_before_any_dataset_write(self) -> None:
        with patch(
            "arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v3_worker.version",
            return_value="0.7.0",
        ):
            with self.assertRaisesRegex(RuntimeError, "requires lerobot==0.6.1"):
                run_v3_worker({"operation": "compose"})

    def test_v3_main_process_atomically_publishes_worker_dataset_and_sidecars(
        self,
    ) -> None:
        source = SnapshotDescriptor(
            "v3",
        ...
        def compose(request):
            ...
            (worker_output / "meta/info.json").write_text(
        ...
        with (
            patch(
                "arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v3._materialize_worker_sources",
                return_value=[],
            ),
            patch(
                "arx5_collection.dataset_pipeline.mining_stage.dataset_generator.recomposition.v3.V3WorkerClient.compose",
                side_effect=compose,
            ),
        ):
    ...
    def test_real_arrow_rewrite_preserves_schema_and_reindexes_only_contract_columns(
        self,
    ) -> None:
...
def _toml(output: Path, sources: list[tuple[str, Path, str]]) -> str:
    ...
    for name, path, selection in sources:
        blocks.extend(
            ["", "[[sources]]", f'name = "{name}"', f'path = "{path}"', selection]
        )
    return "\n".join(blocks) + "\n"
