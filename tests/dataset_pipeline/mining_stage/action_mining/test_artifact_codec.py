...
class SelectionArtifactCodecTest(unittest.TestCase):
    def test_writes_v1_selection_contract(self) -> None:
        ...
        source_schema = json.loads(
            (
                ...
                / "config/specs/schemas/dataset-source-manifest-v1.json"
            ).read_text()
        )
    ...
    def test_writes_equal_eef_v2_provenance(self) -> None:
        ...
        segment = Pi05Segment(0, 0, 1, (sample,))
        selection = DatasetSelection(
            (EpisodeSelection("episode-a", "task", (sample,), (segment,)),), ()
        )
        ...
        sample_schema = json.loads((schema_root / "pi05-sample-v2.json").read_text())
        segment_schema = json.loads((schema_root / "pi05-segment-v2.json").read_text())
        ...
        self.assertEqual(
            report["sampling_contract"]["horizon_semantics"], "trajectory_steps"
        )
