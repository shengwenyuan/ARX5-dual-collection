...
from arx5_collection.adapters.bos.bucketlink import BucketLinkCreated, BucketLinkStatus
...
profile = "builtin:pi05-equal-eef-v3"
...
def test_successful_transfer_enters_existing_conversion_boundary(
    tmp_path: Path,
) -> None:
    config_path, target = write_config(tmp_path)
    sentinel = object()
    ...
    with patch(
        "arx5_collection.dataset_pipeline.execution.bucketlink.execute_dataset_pipeline_config",
        return_value=sentinel,
    ) as execute:
        result = execute_bucketlink_conversion(
            BucketLinkRunRequest(
                config_path, tmp_path / "lerobot/output", "run-1", None
            ),
            TTY("\n"),
            TTY(),
            client=client,
