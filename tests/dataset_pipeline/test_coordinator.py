...
from arx5_collection.dataset_pipeline.configuration.run import OutputConfig
from arx5_collection.dataset_pipeline.configuration.run import PrefetchRuntimeConfig
from arx5_collection.dataset_pipeline.configuration.run import RecipeConfig
from arx5_collection.dataset_pipeline.configuration.run import RuntimeConfig
...
from arx5_collection.dataset_pipeline.execution.coordinator import ConversionWork
from arx5_collection.dataset_pipeline.execution.coordinator import StageWork
from arx5_collection.dataset_pipeline.execution.coordinator import StreamingCoordinator
from arx5_collection.dataset_pipeline.execution.coordinator import StagingCapacityError
from arx5_collection.dataset_pipeline.execution.coordinator import spawn_executor
...
class StreamingCoordinatorTest(unittest.TestCase):
    ...
    def test_closes_jobs_with_bounded_work_and_releases_success_staging(self) -> None:
        ...
        self.assertLess(
            min(
                index
                for index, item in enumerate(started)
                if item.startswith("convert:")
            ),
    ...
    def test_resume_restarts_interrupted_attempt_and_cleans_hidden_partials(
        self,
    ) -> None:
    ...
    def test_default_executor_uses_spawn_context(self) -> None:
        with patch(
            "arx5_collection.dataset_pipeline.execution.coordinator.ProcessPoolExecutor"
        ) as factory:
    ...
    def test_bounded_prefetch_overlaps_independent_stage_and_conversion_pools(
        self,
    ) -> None:
        manifest = self._manifest("episode-a", "episode-b", "episode-c", "episode-d")
    ...
    def test_prefetch_resume_reuses_valid_complete_stage_without_copy(self) -> None:
        ...
        with patch(
            "arx5_collection.dataset_pipeline.execution.coordinator.validate_stage",
    ...
    def test_buffered_prefetch_retained_failure_does_not_act_as_soft_watermark(
        self,
    ) -> None:
        manifest = self._manifest("episode-a", "episode-b")
        retained = manifest.run_dir / "staging" / "episode-a"
    ...
    def test_buffered_resume_quarantines_invalid_stage_then_restages(self) -> None:
        ...
        self.assertEqual(jobs["episode-a"].state, JobState.COMMITTED)
        quarantined = list((manifest.run_dir / "quarantine" / "staging").iterdir())
