...
from arx5_collection.dataset_pipeline.mining_stage.dataset_generator.lerobot_dataset_merge import (
    SnapshotBuildResult,
)
from arx5_collection.dataset_pipeline.configuration.run import DatasetPipelineConfig
from arx5_collection.dataset_pipeline.execution.coordinator import CoordinatorMetric
from arx5_collection.dataset_pipeline.execution.coordinator import CoordinatorProgress
from arx5_collection.dataset_pipeline.source.discovery import discover_episodes
...
class DatasetPipelineApplicationTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_new_run_aligns_then_executes_frozen_pipeline(self) -> None:
        output = _TTY()
        with patch(
            "arx5_collection.dataset_pipeline.application.StreamingCoordinator",
        ...
        self.assertIn("ARX5 dataset pipeline alignment", output.getvalue())
    ...
    def test_cancelled_alignment_has_no_run_side_effect(self) -> None:
        with self.assertRaises(AlignmentCancelled):
            execute_dataset_pipeline(
                DatasetPipelineRequest(self.config_path, run_id="cancelled-run"),
                _TTY("no\n"),
                _TTY(),
                builder=_fake_builder,
                clock=lambda: NOW,
    ...
    def test_resume_skips_discovery_and_explicitly_retries_failed_job(self) -> None:
        ...
        self.assertNotIn("ARX5 dataset pipeline alignment", output.getvalue())
    ...
    def test_request_rejects_ambiguous_run_controls(self) -> None:
        for request, reason in (
            ...
            (
                DatasetPipelineRequest(self.config_path, retry_failed=True),
                "requires --resume",
            ),
            (
                DatasetPipelineRequest(
            ...
            (
                DatasetPipelineRequest(self.config_path, resume_run_id="../escape"),
                "one path component",
            ),
        ):
            with self.subTest(reason=reason):
                with self.assertRaisesRegex(ValueError, reason):
                    execute_dataset_pipeline(request, _TTY(), _TTY())

    def _create_manifest(self, run_id: str) -> RunManifest:
        config = DatasetPipelineConfig.load(self.config_path)
