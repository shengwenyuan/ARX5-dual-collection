...
class LeRobotV21BuilderTest(unittest.TestCase):
    ...
    def test_builds_deterministic_multitask_snapshot_and_releases_run_cache(
        self,
    ) -> None:
...
class _PatchGroup:
    def __init__(self, rewrite) -> None:
        self._rewrite = patch(
            "arx5_collection.dataset_pipeline.mining_stage.dataset_generator.lerobot_dataset_merge._rewrite_episode_parquet",
            side_effect=rewrite,
        )
        self._validate = patch(
            "arx5_collection.dataset_pipeline.mining_stage.dataset_generator.lerobot_dataset_validator.validate_lerobot",
